...
func TestInfrastructure(t *testing.T) {
	...
	terraformDir := "C:/Users/akhilesh.pal/Desktop/ORDER_MANAGEMENT_USECASE_TERRAFORM/ORDER_MANAGEMENT_USECASE_TERRAFORM"
...
func validateDynamoDBTable(t *testing.T, terraformOptions *terraform.Options) {
	...
	dynamodbTableName := terraform.Output(t, terraformOptions, "OM_USECASE_DynamoDBTable_Name")

	// Add assertions for the DynamoDB Table
	assert.NotNil(t, dynamodbTableName, "DynamoDB table name should not be nil")
	assert.Contains(t, dynamodbTableName, "OM_USECASE_DynamoDBTable", "DynamoDB table name should be 'OM_USECASE_DynamoDBTable'")
	...
	primaryKeyAttributeType := terraform.Output(t, terraformOptions, "OM_USECASE_DynamoDBTable_HashKey_Type")
	assert.Equal(t, expectedPrimaryKeyType, primaryKeyAttributeType, "DynamoDB Primary Key attribute type should be 'N' (Number)")

	dynamoDBHashKey := terraform.Output(t, terraformOptions, "OM_USECASE_DynamoDBTable_HashKey")
	...
	dynamoDBBillingMode := terraform.Output(t, terraformOptions, "OM_USECASE_DynamoDBTable_Billing_Mod")
...
func validateLambdaFunction(t *testing.T, terraformOptions *terraform.Options) {
	...
	lambdaFunctionName := terraform.Output(t, terraformOptions, "OM_USECASE_LambdaFunction_CRUD_Function_Name")
	GetlambdaFunctionName := terraform.Output(t, terraformOptions, "OM_USECASE_GetFunction")
	lambdaFunctionRuntime := terraform.Output(t, terraformOptions, "OM_USECASE_LambdaFunction_Run_time")

	// Add assertions for the Lambda function
	assert.NotNil(t, lambdaFunctionName, "Lambda function name should not be nil")
	assert.Equal(t, "OM_USECASE_LambdaFunction_CRUD", lambdaFunctionName, "Lambda function name should be 'OM_USECASE_LambdaFunction_CRUD'")

	assert.NotNil(t, GetlambdaFunctionName, "Lambda function name should not be nil")
	assert.Equal(t, "OM_USECASE_LambdaFunction_GET", GetlambdaFunctionName, "Lambda function name should be 'OM_USECASE_LambdaFunction_GET'")
...
func validateSQSQueue(t *testing.T, terraformOptions *terraform.Options) {
	sqsQueueName := terraform.Output(t, terraformOptions, "OM_USECASE_QueueSQSName")
	// Retrieve the SQS Queue URL from Terraform output
	sqsQueueURL := terraform.Output(t, terraformOptions, "OM_USECASE_QueueSQS_URL_")
	assert.NotNil(t, sqsQueueName, "SQS queue name should not be nil")
	assert.Equal(t, "OM_USECASE_QueueSQS", sqsQueueName, "Queue name should be 'OM_USECASE_QueueSQS'")

	// Add assertions for the SQS Queue URL format
	assert.NotNil(t, sqsQueueURL, "SQS Queue URL should not be nil")
	assert.Regexp(t, regexp.MustCompile("^https://sqs\\.us-east-1\\.amazonaws\\.com/your_aws_id/OM_USECASE_QueueSQS$"), sqsQueueURL, "SQS Queue URL should match the expected format")
}
...
func validateAPIGateway(t *testing.T, terraformOptions *terraform.Options) {
	...
	apiGatewayName := terraform.Output(t, terraformOptions, "OM_USECASE_APIGateway_Name")
	apiGatewayARN := terraform.Output(t, terraformOptions, "OM_USECASE_APIGateway_Arn")

	// Add assertions for API Gateway name
	assert.NotNil(t, apiGatewayName, "API Gateway Name should not be nil")
	assert.Equal(t, "OM_USECASE_APIGateway", apiGatewayName, "API Gateway Name should match the expected name")
	...
	assert.Equal(t, "ORDER_MANAGEMENT_USECASE", apiGatewayTags["Name"], "Name tag should be 'ORDER_MANAGEMENT_USECASE'")
